    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (exito) {
                ...
                // Necesitamos recuperar la tesis para saber quién es el docente
                Tesis tesis = dao.getTesisPorId(idTesis); // Este método necesita traer el código del docente
                
                // Si getTesisPorId no trae el código del docente, podemos usar getTesisPorEstudiante
                // Pero getTesisPorId ya lo implementamos en TesisDAO, asegurémonos de que traiga el código docente revisor
                
                // Una forma más directa si TesisDAO.getTesisPorId no trae el código:
                // Recuperamos la tesis completa por estudiante (ya que sabemos el estudiante actual)
                Tesis tesisCompleta = dao.getTesisPorEstudiante(usuario.getCodigo());
                
                if (tesisCompleta != null && tesisCompleta.getCodigoDocenteRevisor() != null) {
                    String codigoDocente = tesisCompleta.getCodigoDocenteRevisor();
                    String mensaje = "El estudiante " + usuario.getNombreCompleto() + " ha subido una nueva versión de su tesis.";
                    if (comentario != null && !comentario.isEmpty()) {
                        mensaje += " Nota: " + comentario;
                    }
                    notificacionService.notificarCambioEstado(codigoDocente, mensaje); // Reutilizamos este método genérico
                }
